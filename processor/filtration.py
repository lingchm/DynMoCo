...
def build_community_graph(Q: torch.Tensor, W: sp.coo_matrix):
    ...
    # Move adjacency to the same device as Q for consistent operations
    target_device = Q.device
    if not torch.is_tensor(W):
        W = torch.tensor(W.todense(), dtype=torch.float, device=target_device)
    else:
...
def WRCF_Index(G, dim, card):
    ...
    # networkx >= 3.0 removed from_numpy_matrix; use from_numpy_array for compatibility
    st = wrcf(nx.from_numpy_array(G))
    dgm = st.persistence()
    ...
    def forward(self, G: torch.Tensor):
        ...
        if d > 0:
            ...
            dgm = torch.cat(
                ...
                dim=1)
        # Return on the same device as input G for consistency
        return dgm.to(G.device)
